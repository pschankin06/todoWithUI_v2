...
parcelHelpers.export(exports, "searchInList", ()=>searchInList);
...
function addTasksFromScreen() {
    for (let task of (0, _viewJs.UI_ELEMENTS).TASKS_ON_SCREEN){
        const TASK_PROPERTIES = {
            HAS_CLASS: task.classList.contains("template"),
            TASK_NAME: task.querySelector(".task__text").textContent,
            TASK_STATUS: task.querySelector(".task__checkbox").classList.contains("task__checkbox--done") ? STATUSES.DONE : DEFAULT.STATUS,
            TASK_PRIORITY: task.closest(".form")
        };
        if (!TASK_PROPERTIES.HAS_CLASS) list.push({
...
function searchInList(taskName) {
    const isTaskInList = list.find((item)=>item.name === taskName.value);
    return isTaskInList;
}

},{"./view.js":"2GA9o","../node_modules/date-fns":"9yHCA","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"2GA9o":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
for (let button of UI_ELEMENTS.DELETE_BUTTONS)button.addEventListener("click", deleteTaskUi);
for (let button1 of UI_ELEMENTS.CHECKOUT_BUTTONS)button1.addEventListener("click", checkout);
for (let form of UI_ELEMENTS.FORMS)form.addEventListener("submit", createTask);
function deleteTaskUi(event) {
    ...
    (0, _mainJs.deleteTask)(name);
    taskBox.remove();
}
function checkout(event) {
    const currentTask = event.currentTarget;
    const name = currentTask.nextElementSibling.textContent;
    const status = currentTask.classList.contains("task__checkbox--done") ? (0, _mainJs.STATUSES).TO_DO : (0, _mainJs.STATUSES).DONE;
    console.log(name);
    console.log(status);
    (0, _mainJs.changeStatus)(name, status);
...
function createTask(event) {
    try {
        ...
        const isTaskInList = (0, _mainJs.searchInList)(taskName);
        const isNotValid = taskName.value === "" || isFinite(taskName.value) || isTaskInList;
        if (isNotValid) {
            clearInput(taskName);
            throw new Error;
        } else {
            (0, _mainJs.addTask)(event, taskName.value);
...
function addTaskText(taskName, newTaskText) {
    newTaskText.textContent = `${(0, _dateFns.format)(Date.now(), "yyyy-MM-dd")} ${taskName.value}`;
}
...
},{"./main.js":"bDbGG","../node_modules/date-fns":"9yHCA","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"9yHCA":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["3Unyy","bDbGG"], "bDbGG", "parcelRequirec28a")
